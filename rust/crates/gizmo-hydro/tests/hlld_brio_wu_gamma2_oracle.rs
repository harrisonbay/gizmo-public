...
const GAMMA: f64 = 2.0;
const ORACLE: &str =
    include_str!("../../../../validation/oracles/briowu/hlld_gamma2_flux_oracle.csv");

#[test]
fn gamma2_hlld_interfaces_match_corrected_c() {
    for (line_number, line) in ORACLE.lines().enumerate().skip(1) {
        ...
        for (component, (&actual, &expected)) in actual.iter().zip(&values[18..33]).enumerate() {
